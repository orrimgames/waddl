// Pip v0.5 - v0.4 + servo fit pass: pockets now model the real SG90 - body
// cavity, mounting-tab plate relief, tab screw pilots, and a cable-exit
// notch (the old pocket was a plain box; the tabs had nowhere to go).
// Everything else unchanged from v0.4.
// Open in OpenSCAD, set PART, F6, export STL. Units mm. No supports at 0.2mm layers.
$fn = 72;

PART = "assembly"; // assembly | shell_front | shell_back | face_bezel | board_tray | mic_shield
                   // foot_left | foot_right | wing_left | wing_right | beak | fit_test
...
// SG90: body 22.5x12.2x22.9, tab plate ~32.2x12.5x2.4 sitting ~6mm below
// the horn face, ear holes ~28mm apart; cable exits the bottom rear edge.
// Horn: 25T spline, ~5.5 dia neck.
sg90      = [23.4, 13.2, 24];   // body + 0.9-1.1mm print clearance
sg90_tab  = [33.4, 13.6, 3.2];  // tab plate relief
tab_z     = -6;                 // tab plane offset from horn-face end
tab_hole  = 14;                 // ear hole half-spacing (28mm apart)
...
module shell_half(front = true) {
    difference() {
        ...
        for (s = [-1, 1]) // foot servo pockets, horns down
            translate([s*hip_x, 12, 0]) rotate([0, 90, 0]) servo_pocket();
        for (s = [-1, 1]) // wing servo pockets, horns out
            translate([s*(body_r*0.62), body_z + 4, 0]) rotate([0, 0, 90]) servo_pocket();
        ...
        // v0.4: mic pinhole array (Tier A INMP441 sits inside behind these)
        if (front) for (i = [-2:2]) for (j = [-1:1])
            translate([i*3.2, 18 + j*3.2, body_r*0.80]) rotate([35, 0, 0]) cylinder(d = 1.2, h = 12);
        // v0.4: wire pass-through slot at the seam (servo/mic leads)
        translate([0, body_z - 30, 0]) cube([14, 6, 6], center = true);
    }
    for (x = [-1, 1]) for (y = [-1, 1])
...
module face_bezel() {
    ...
    // v0.4: two OLED retention clips - the 128x64 module slides in from behind
    for (s = [-1, 1])
        translate([s*14, -2, 6]) difference() {
            cube([3, 26, 6], center = true);
            translate([0, 0, -2]) cube([4, 28, 2.6], center = true);  // board slides over this lip
        }
}
// v0.4: optional mic shield - tiny grille cup that holds the INMP441 behind
// the pinhole array, prints standing on its rim.
module mic_shield() {
    difference() {
        cylinder(d = 14, h = 6, center = true);
        cylinder(d = 11, h = 8, center = true);
        translate([0, 0, -3]) cube([16, 4, 3], center = true);  // wire exit
    }
}

// ---- v0.5: real-SG90 pocket ----
// Local frame: horn face toward +Z end, cable exits bottom (-Z) rear (-Y).
// Rotate/translate at the call site. Self-tap the servo ears into the 2.1mm
// pilots, or just press-fit: the cavity alone holds an SG90 snugly.
module servo_pocket() {
    cube(sg90, center = true);                                        // body
    translate([0, 0, sg90.z/2 + tab_z])
        cube(sg90_tab, center = true);                                // tab plate
    for (s = [-1, 1])                                                 // ear screw pilots
        translate([s*tab_hole, 0, sg90.z/2 + tab_z])
            cylinder(d = 2.1, h = sg90_tab.z + 2, center = true);
    translate([0, -(sg90.y/2 + 1), -(sg90.z/2 - 2)])
        cube([8, 8, 7], center = true);                               // cable exit notch
}

// v0.5: print-me-first gauge - one pocket in a flat coupon. 15 min print
...
else if (PART == "mic_shield")  mic_shield();
